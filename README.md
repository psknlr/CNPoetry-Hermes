...
飞花令，在 Colab 中直接打开中国风 Web 控制台，并可经 **ngrok 一键映射为公开链接**分享。
...
| **B 计量** | 确定性算法可复算的形式事实 | 句式/体裁判定/韵脚位置/韵伴聚类/**平仄与韵部（依《广韵》）** |
...
# 作品全息（A原文/B计量/C旁证/D外部分析/互文）；同题多作用 @作者 消歧
...
支持**全部例证作品浏览** · 词牌定格——**龙榆生《唐宋词格律》权威谱 + 语料归纳
双层互证，同调异名归并，全部例词可回源** · 诗人档案——**支持字/号/别称：
苏东坡→苏轼、稼轩→辛弃疾**）· **卷四 创作·游艺**（创作实验室：四起式标准谱／
龙谱词牌 + 平水韵候选字 + 意象点选 + 草稿平仄/律则/撞句复核 · **古风长篇**：
歌行体 AI 智能体创作，检索规则组合成换韵方案，参照长恨歌等语料范例，代拟稿过
形式核验 · **飞花令**：对句须语料实有，逐字回源判定，参照中国诗词大会规则）·
**卷五 研究端**（韵伴聚类——**实押例句可点击回源** · 互文检测 · 统计资产 · Skill 库）。

| 词牌定格（龙谱 + 语料互证） | 飞花令（逐字回源判定） |
|---|---|
| ![词牌定格](docs/assets/ui-cipai-light.png) | ![飞花令](docs/assets/ui-feihua-light.png) |
...
### CitationGuard 三重核验 + ClaimGuard 论断核验

CitationGuard（引文真实性）：
- **存在性**：被引 poem_id 必须在语料中，否则标记「疑似杜撰」；
- **引文**：引号内诗句必须**逐字**（简繁折叠后）存在于被引作品——改一字
  也会被抓住，相似度只作诊断不作放行；并做就近归属检查；
- **取证闭环**：本轮工具证据之外的真实编号同样违规（**存在≠取证**；
  传入空证据列表即 fail-closed，猜中的真实编号也不放行）。

ClaimGuard（论断真实性，第二道闸门）：引文真实不等于推论成立。答案被拆为
分型论断核验——作者/朝代归属查库（「《静夜思》是杜甫所作」→ 违例并给出
正确归属）、句数等数字复算、对古人心理状态的强断言与无证据无限定语的
强解释标记「过度阐释风险」。启发式下界核验，边界如实声明。

## 接入真实大模型（可选）

**方式 A：原生直连（零依赖，标准库 urllib，支持模型自主工具调用）**

```bash
# Azure OpenAI
export HERMES_LLM_BACKEND=azure
export AZURE_OPENAI_ENDPOINT=https://<资源名>.openai.azure.com
export AZURE_OPENAI_API_KEY=…
export HERMES_LLM_MODEL=<部署名>
...
`HERMES_LLM_BACKEND=auto`（默认）按 专属直连变量 → litellm → local 的顺序解析。
智能体是**模型自主思考选择工具**的 ReAct 循环（有界步数与工具预算），全部生成
内容过引用核验 + 论断核验双闸门。无 key 时自动使用**确定性 local 后端**：与真实
模型走完全相同的工具循环与输出模式，全功能离线可跑、可测试、逐字节可复现；
真实模型调用失败时优雅回退并在文末标注，回退产物绝不写入真实后端缓存。
...
- **③ gujilab/chinese-classical-corpus（C 层训诂，`data/raw/gujilab/`，CC0）**：
  ...
  自动命中繁体字头。
- **④ 《广韵》字音表（B 层音韵，`data/raw/ytenx/`，韵典网整理本）**：
  25,333 字条 / 3,874 小韵（声母/韵目/反切），驱动平仄标注（多音字标
  「两读」）、韵目归部、三平尾/失粘/失对等近体律则检测与韵伴聚类的
  声调纯度交叉验证；206 韵四声由内嵌规范表判定，装载时 fail-loud。
- **⑤ 龙榆生《唐宋词格律》（词谱权威层，`data/raw/longyusheng/`）**：
  153 调结构化词谱（调名异名/类别/说明/定格变格符号谱，○平●仄⊙可平可仄
  △平韵▲仄韵），驱动词牌定格「权威谱 + 语料归纳」双层互证与创作实验室
  词牌模式；同调异名（忆江南=望江南=梦江南=江南好）自动归并。
...
> 口径声明：grounding 衡量**检索落地与引用编排正确性**——local 为确定性
> 后端，其高分不代表语言模型的诗学理解能力；解释质量评测（人评/LLM 评）
> 见 [docs/ROADMAP.md](docs/ROADMAP.md) 的 CNPoetryBench 计划。
...
OpenCC Apache-2.0；韵典网广韵整理本；龙榆生《唐宋词格律》整理数据；
PoetryMTEB 样本按其数据集条款；gujilab CC0）。

本项目由**医哲未来人工智能研究院（IMPF-AI）**研发，架构承继
Shanghan-Hermes（伤寒-赫尔墨斯）「证据优先」范式。
