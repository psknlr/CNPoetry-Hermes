...
| **C 旁证** | 集内注释、作者小传、白话导读 | 花间集/南唐二主词 notes、全唐诗小传、水墨唐诗 |
| **D 外部分析** | 外部数据集 LLM 生成（非本系统结论） | PoetryMTEB（DeepSeek-V3.1 分析层）样本 |
...
- **种子语料（随仓库提交，`data/raw/`，约 14MB）**：
  ...
  唐宋作者小传；OpenCC `TSCharacters.txt`（简繁归一字表，Apache-2.0）；
  [PoetryMTEB/ChineseClassicalPoetryDatabase](https://huggingface.co/datasets/PoetryMTEB/ChineseClassicalPoetryDatabase)
  分析层样本 1900 条（D 层，标注「外部 LLM 生成」）。
...
- **扩充**：`python3 -m hermes_poetry fetch` 可下载全唐诗全量（5.7 万首）、
  全宋词全量等；[gujilab/chinese-classical-corpus](https://huggingface.co/datasets/gujilab/chinese-classical-corpus)
  （十三经+古译今指令对，CC0）作为旁证层扩展方向，见 `fetch --list`。

## 规则库规模（种子语料一次 pipeline 后）

| 资产 | 规模 |
|---|---|
| 归一化作品 | 26,276 首（跨集去重后；重出互见并入 `also_in`） |
| 初始规则（过闸门） | 47,921 条（拒绝 822 条，全量审计 289,214 条） |
| 意象档案 | 50 个（如「月」786 首支撑，相反情感并存如实呈现） |
| 题材/词牌/诗人档案 | 10 / 305 / 484 个 |
| 韵伴聚类 | 21 组（纯语料归纳，与平水韵高度吻合但不冒称权威） |
| 互文规则 | 4,000 条（重出互见/袭用/化用） |
| 编译 Skill | 618 个 |
...
| 套件 | 口径 | 结果 |
|---|---|---|
| retrieval | 每首取最长一句作查询，命中原诗 | Top-1 96.0%，Top-5 98.7%，MRR 0.973 |
...
```bash
python3 -m unittest discover -s tests    # 50 项，含对抗性证据注入测试
```
