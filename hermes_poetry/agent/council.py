...
class Council:
    ...
    def deliberate(self, question: str, role: str = "") -> Dict:
        ...
        timeline.append(CouncilMessage("Planner", "规划者", "plan",
                                       f"派遣专家：{'、'.join(_SPECIALISTS[s]['cn'] for s in plan['specialists'])}",
                                       data=plan))
        # 取证员
        ...
        for name in plan["specialists"]:
            ...
            timeline.append(CouncilMessage(name, spec["cn"], "analyze",
                                           comment or f"以 {spec['tool']} 取证 {len(ids)} 处",
                                           evidence_ids=ids, data=result))
        # 批评者
        critic_flags = []
        for j in judgments:
            if j["has_error"]:
